...
var Cache = function () {
  ...
  _createClass(Cache, [{
    ...
    value: function checkForUpdates() {
      ...
      var indexes = Object.keys(this.handles).filter(_utils.isNumeric).map(function (n) {
        return +n;
      });
    ...
    value: function sendUpdate(url, handler, data) {
      ...
      var isError = (0, _utils.isErrorObject)(data);
      urls.forEach(function (urlObj) {
        if (urlMatches(urlObj)) {
          ...
          var resData = root === false ? data : (0, _utils.getObjectValue)(root, data, defaultValue);
    ...
    value: function processUpdate() {
      ...
      var now = new Date();
      var key = this.getCacheKey(method, url);
      var keys = Object.keys(this.handles).filter(_utils.isNumeric).map(function (n) {
        return +n;
      });
    ...
    value: function fetch(_ref) {
      ...
      if ((typeof sourceHeaders === 'undefined' ? 'undefined' : _typeof(sourceHeaders)) === 'object') {
        options.headers = Object.keys(sourceHeaders).reduce(function (h, key) {
          var value = sourceHeaders[key];
          h[key] = typeof value === 'function' ? value() : value;
          return h;
        }, {});
      }
      return (0, _utils.fetchJson)(_extends({
        url: url,
        method: method
      }, options, {
    ...
    value: function watch() {
      ...
      var urls = (0, _utils.flatten)(Object.keys(options).map(function (key) {
        var opts = options[key];
        var url = opts.url;
